module SessionsHelper
  ...
  def correct_user        
    begin
	  ...
	  if !current_user?(merchant)
        gflash :warning => { :value => "Access denied!", :nodom_wrap => true }		
        redirect_to merchant_path(current_user)
      end	  
    rescue ActiveRecord::RecordNotFound
        gflash :warning => { :value => "Merchant with such id was not found!", :nodom_wrap => true }
        redirect_to merchant_path(current_user)	
    end   	
  end
  ...
  private
    ...
    def user_from_remember_token
      Merchant.find_by_id(cookies.signed[:remember_token])
    end       
